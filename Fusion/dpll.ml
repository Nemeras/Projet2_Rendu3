...
module Solve (E : Clauses) =
struct


module M = Init_clauses (E)
module S = Step (E)
module Bcp = Propagation (E)
module P = Print (E)
...
let loop cnf stack clauses current pos origins solution levels orders k para compt =
	...
	while abs !k <= cnf.v_real && !k <> 0 do
		...
		if solution.(0) = 0 then
			Bcp.propa stack current pos solution levels orders para ;
		
		(* Si toutes les variables ont été instanciées *)
		if abs !k = cnf.v_real then
			(* S'il y a contradiction : backtrack *)
			if solution.(0) < 0 then
				S.continue stack clauses current pos origins solution levels orders k para
			(* Sinon : c'est fini *)
			else
				k := cnf.v_real + 1
		(* Sinon : on continue *)
...
let solve cnf wl learning draw unsat print =
	...
	loop cnf stack clauses current pos origins solution levels orders k para compt ;
