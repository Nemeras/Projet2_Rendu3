...
module Learning_p (E : Clauses) (T : Theory) =
struct
...
let build_graph current solution levels orders level activate =
	...
	if activate then
		...
		E.edges (-solution.(0)-1) graph current solution levels orders seen 0 level v
		end ;
	graph
...
let from_scratch stack solver current pos solution levels k para =
	...
	if (!k != 0) then
		...
		while !k != 0 do
			...
			solution.(abs !k) <- 0 ;
			k := E.backtrack stack current pos levels ;
...
let learning stack solver clauses current pos solution levels orders k para origins =
	...
	if para.learning then
		...
		if x = -1 then
			...
			from_scratch stack solver current pos solution levels k para
		else
