...
let learning stack clauses current solution levels orders k para origins =
	...
	if para.learning then
		...
		let clause_mod = Stack.maj_clause_learning stack new_clause levels in
		...
		if x = -1 then
			...
			Stack.from_scratch stack current solution levels k para
		else
