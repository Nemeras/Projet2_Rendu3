...
(* Indique si x est dans le Set para.heap *)
let is_in x para =
	mem (score x para, x) para.heap


(* Ajoute x dans le Set *)
let add_score x para =
	para.heap <- add (score x para, x) para.heap


(* Enlève x du Set *)
let remove_score x para =
	para.heap <- remove (score x para, x) para.heap


(* Initie para.scores et para.heap *)
let create pos para =
...
let decr_scores para =
	for i = 1 to (Array.length para.scores)/2 do
		modif i (int_of_float ((float_of_int (score i para))*.0.95)) para ;
		modif (-i) (int_of_float ((float_of_int (score (-i) para))*.0.95)) para
	done
