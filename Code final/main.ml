...
struct
	...
	let init file wl learning vsids draw unsat print aff_cnf =
	
		let cnf, solver = T.create file aff_cnf in
	
		let module C = (val (version wl) : Clauses) in
		let module S = Dpll.DPLL (C) (T) in
	
		let res = S.solve cnf solver learning vsids draw unsat print in
...
let _ =
	...
	let vsids = ref false in	(* True ssi l'heuristique VSIDS est activée *)
	...
	let options = [
		...
		("-vsids", Arg.Unit (fun () -> learning := true ; vsids := true), "Active l'heuristique VSIDS.") ;
	...
	let module T = (val (choose_theory !theory) : Theory) in	(* Thérie utilisée *)
	let module Launch = Init_theory (T) in
	Launch.init !file !wl !learning !vsids !draw !unsat !print !cnf	(* Lancement de l'algorithme *)
